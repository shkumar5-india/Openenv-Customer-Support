...
def log_step(step: int, action: str, reward: float, done: bool, error: Optional[str]) -> None:
    ...
    print(f"[STEP] step={step} action={action_clean!r} reward={reward:.2f} done={done_str} error={error_str}", flush=True)
def log_end(success: bool, steps: int, score: float, rewards: List[float]) -> None:
    success_str  = "true" if success else "false"
    rewards_str  = ",".join(f"{r:.2f}" for r in rewards)
    print(f"[END] success={success_str} steps={steps} score={score:.2f} rewards={rewards_str}", flush=True)
def server_reset(task: str) -> Dict[str, Any]:
    resp = requests.post(f"{SERVER_BASE_URL}/reset", params={"task": task}, timeout=30)
    resp.raise_for_status()
    return resp.json()
def server_step(action_payload: Dict[str, Any]) -> Dict[str, Any]:
    resp = requests.post(f"{SERVER_BASE_URL}/step", json=action_payload, headers={"Content-Type": "application/json"}, timeout=60)
    resp.raise_for_status()
    return resp.json()
def call_llm(observation: Dict[str, Any], task_name: str) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "  predicted_category (delivery, refund, complaint)\n"
...
def _rule_based_action(ticket_text: str, task_name: str, category_hint: Optional[str], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    responses = {
        "delivery": "We apologize for the delivery issue. We're checking the shipment status and will update you soon.",
        "refund": "Your refund request has been received and will be processed within 5-7 business days.",
        "complaint": "We’re sorry for your experience. We’re investigating and will resolve this promptly.",
    }
    return {
...
def run_episode(task_name: str) -> Dict[str, Any]:
    ...
    while not done:
        ...
        log_step(step=step_num, action=str(action_dict.get("response_text", "")), reward=reward, done=done, error=error_msg)
    ...
    return {"task": task_name, "success": success, "steps": step_num, "score": final_score, "rewards": rewards}
def main() -> None:
    print("=" * 60, flush=True)
    print("OpenEnv Customer Support — Runner", flush=True)
    ...
    for r in all_results:
        ...
        print(f"[{status}] task={r['task']} steps={r['steps']} score={r['score']:.2f} rewards=[{rewards_str}]", flush=True)
        total_score += r["score"]
